...
/**
 * IOC-2: Phát hiện outbound TCP connections từ node process trong lúc runtime.
 *
 * Windows: netstat -nob hoặc PowerShell Get-NetTCPConnection
 * Linux:   ss -tnp (hiển thị process name kèm connection)
 */
...
/**
 * Linux: dùng `ss -tnp` để lấy ESTABLISHED connections có process node
 */
function checkLinuxConnections() {
  const alerts = [];
  try {
    // ss -tnp: TCP, numeric, show process
    const output = execSync("ss -tnp state established 2>/dev/null", {
      encoding: "utf8",
      timeout: 10000,
    });
    ...
    for (const line of lines) {
      // Chỉ lấy dòng có "node" trong phần process
      if (!line.includes("node")) continue;

      // Format: ESTAB  0  0  local:port  remote:port  users:(("node",pid=xxx,...))
      const connMatch = line.match(/\S+\s+\d+\s+\d+\s+\S+\s+(\d+\.\d+\.\d+\.\d+):(\d+)/);
      ...
      if (!isLocal && !isVerdaccio) {
        log("ALERT", `[IOC-2] node has outbound connection to ${remoteIP}:${remotePort} (ESTABLISHED)`);
        alerts.push({ process: "node", remote: `${remoteIP}:${remotePort}`, state: "ESTABLISHED" });
      }
    }
  } catch (err) {
    log("WARN", `[IOC-2] ss command failed: ${err.message}`);
  }

  // Kiểm tra thêm /proc/net/tcp để tìm connection đang tồn tại sau postinstall
  try {
    const netTcp = require("fs").readFileSync("/proc/net/tcp", "utf8");
    const lines  = netTcp.split("\n").slice(1).filter(Boolean);
    for (const line of lines) {
      const parts = line.trim().split(/\s+/);
      if (parts[3] !== "01") continue; // 01 = ESTABLISHED
      const remoteHex = parts[2];
      const [ipHex, portHex] = remoteHex.split(":");
      // Convert little-endian hex IP
      const ip = ipHex.match(/../g).reverse().map(b => parseInt(b, 16)).join(".");
      const port = parseInt(portHex, 16);
      const isLocal     = ip.startsWith("127.") || ip === "0.0.0.0";
      const isVerdaccio = port === 4873;
      if (!isLocal && !isVerdaccio && port === 8080) {
        log("ALERT", `[IOC-2] /proc/net/tcp: suspicious ESTABLISHED connection to ${ip}:${port}`);
        if (!alerts.find(a => a.remote === `${ip}:${port}`)) {
          alerts.push({ process: "node", remote: `${ip}:${port}`, state: "ESTABLISHED (proc)" });
        }
      }
    }
  } catch (_) { /* /proc not available */ }
...
function checkNodeNetworkConnections() {
  log("INFO", "[IOC-2] Checking outbound connections from node process...");

  // Linux path
  if (os.platform() === "linux") {
    const alerts = checkLinuxConnections();
    if (alerts.length === 0) {
      log("INFO", "[IOC-2] No suspicious outbound connections found.");
    }
    return alerts;
  }
  ...
  if (os.platform() !== "win32") {
    log("WARN", "[IOC-2] Platform not supported.");
    return [];
  }
